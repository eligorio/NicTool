...
sub valid_password_ldap {
    my ($self, $attempt, $db_pass, $user, $group, $salt) = @_;
    ...
    my $bind_pass = $self->get_option('ldap_bindpw');
    ...
    # select Manager DN pattern
    my $ldap_result = $ldap->bind( $bindDN, password => $bind_pass );
